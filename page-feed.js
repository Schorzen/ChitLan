...
import { createPost, listenToFeed, listenToReplies, addReply, reportPost, setPostReaction, POST_TYPES } from './feed.js';
...
// postId -> { unsub, lastReplies } — lastReplies lets an expanded section
// be restored instantly after any full feed re-render (e.g. from a
// reaction or reply-count change on ANY post), instead of collapsing.
...
function reactionsHTML(post) {
  ...
  return `<div class="msg-reactions" style="margin-bottom:var(--space-3);">${Object.entries(grouped).map(([emoji, info]) => `
    <button class="reaction-pill ${info.mine ? 'mine-reaction' : ''}" data-emoji="${emoji}" data-action="quick-react">${emoji} ${info.count}</button>
  `).join('')}</div>`;
}
  ...
  return `
    <div class="feed-post-card" data-post-id="${post.id}">
      ...
      <div class="feed-post-footer">
        <button class="feed-reply-toggle" data-action="react">${icon('heart', { size: 14 })} React</button>
...
function restoreExpandedReplies() {
  expandedPosts.forEach((state, postId) => {
    const repliesEl = document.getElementById(`replies-${postId}`);
    if (!repliesEl) return; // post filtered out of the current view
    repliesEl.classList.remove('hidden');
    renderReplies(postId, state.lastReplies || []);
  });
}
...
function renderReplies(postId, replies) {
  const state = expandedPosts.get(postId);
  if (state) state.lastReplies = replies;
...
function toggleReplies(postId, card) {
  ...
  if (expandedPosts.has(postId)) {
    ...
    expandedPosts.set(postId, { unsub, lastReplies: [] });
  }
}

async function toggleReaction(post, emoji) {
  try {
    await setPostReaction(post.id, emoji, user.uid, post.reactions || {});
  } catch (e) {
...
function openReactionPicker(post) {
  ...
  const myReaction = post.reactions?.[user.uid];
  backdrop.innerHTML = `
    <div class="msg-menu">
      <div class="reaction-row">
        ${QUICK_REACTIONS.map((e) => `<button class="reaction-option ${e === myReaction ? 'selected' : ''}" data-emoji="${e}">${e}</button>`).join('')}
      </div>
      <button id="cancel-react-action">${icon('close', { size: 18 })} Cancel</button>
    </div>`;
  document.body.appendChild(backdrop);
  backdrop.querySelectorAll('.reaction-option').forEach((btn) => {
    btn.addEventListener('click', async () => {
      await toggleReaction(post, btn.dataset.emoji);
      backdrop.remove();
    });
  });
  backdrop.querySelector('#cancel-react-action').addEventListener('click', () => backdrop.remove());
...
document.getElementById('feed-list').addEventListener('click', (e) => {
  ...
  const quickReact = e.target.closest('[data-action="quick-react"]');
  if (quickReact) {
    toggleReaction(post, quickReact.dataset.emoji);
    return;
  }
