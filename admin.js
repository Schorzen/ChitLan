...
// Searches by username OR email — detects which one you typed based on
// whether it contains "@". Still an exact match under the hood (Firestore
// doesn't do partial/fuzzy text search without extra tooling), but this at
// least means "I don't remember their exact username" isn't a dead end
// anymore if you have their email instead.
export async function searchUsers(queryText) {
  ...
  const q = query(collection(db, 'users'), where(field, '==', value), limit(10));
  const snap = await getDocs(q);
  return snap.docs.map((d) => ({ id: d.id, ...d.data() }));
}

export async function listBlockedUsers() {
  const q = query(collection(db, 'users'), where('isBlocked', '==', true), limit(50));
  const snap = await getDocs(q);
  return snap.docs.map((d) => ({ id: d.id, ...d.data() }));
}

// Total reports ever filed against a user (open + resolved + dismissed),
// used to flag repeat offenders in the open-reports list.
export async function getReportCountForUser(uid) {
  const q = query(collection(db, 'reports'), where('reportedUser', '==', uid));
