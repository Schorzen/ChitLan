...
const BASE_CONTENT_PADDING = 'calc(var(--nav-height) + 78px)';
const REPLYING_CONTENT_PADDING = 'calc(var(--nav-height) + 78px + 52px)';
...
function startReply(message) {
  ...
  pageContent.style.paddingBottom = REPLYING_CONTENT_PADDING;
  document.getElementById('chat-input').focus();
}
function cancelReply() {
  replyingTo = null;
  document.getElementById('reply-preview-bar').classList.add('hidden');
  pageContent.style.paddingBottom = BASE_CONTENT_PADDING;
}
