...
import {
  ...
  updateDoc, increment, serverTimestamp, Timestamp, writeBatch, deleteField,
...
// One reaction per person per post, same model as chat message reactions —
// tapping the same emoji again removes it, a different one swaps it.
export async function setPostReaction(postId, emoji, uid, currentReactions = {}) {
  const alreadyThisEmoji = currentReactions[uid] === emoji;
  const value = alreadyThisEmoji ? deleteField() : emoji;
  await updateDoc(doc(db, POSTS_COL, postId), { [`reactions.${uid}`]: value });
}
